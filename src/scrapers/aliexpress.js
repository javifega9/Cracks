...
async function scrapeAliExpress(query) {
  return withPage(async (page) => {
    ...
    await page.waitForSelector('a[href*="/item/"], a[href*="aliexpress.com/item/"]', {
      timeout: 6000
    }).catch(() => {});

    return page.evaluate((limit) => {
      const cards = Array.from(document.querySelectorAll('a[href*="/item/"], a[href*="aliexpress.com/item/"]'));
      const seen = new Set();
      const results = [];
      ...
      for (const card of cards) {
        ...
        const oldPriceMatch = text.match(/([\d,.]+)\s*€\s*([\d,.]+)%/) || null;
